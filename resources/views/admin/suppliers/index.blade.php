...
@section('content')
  ...
  <div class="cover-input-group">
    <div class="input-group fl">
      <input type="text" id="search-text" class="form-control" placeholder="Nama/Alamat" value="{{ $search }}"> 
    </div>
    <div class="input-group fl">
      <input type="button" name="search" value="Search" id="button-search" data-loading-text="Loading..." class="btn btn-primary"> 
    </div>
	<div class="input-group fl">
		<a class="btn btn-success" href="{{url('add-suppliers', $parameters = [], $secure = null)}}"><i class="glyphicon glyphicon-plus"></i> Add</a>
	</div>
    <?php //if ($user->level==1) { ?>
  ...
  <script>
    $(document).ready(function(){
		prepareData('<?php echo url('load-list-suppliers'); ?>');
    });
  </script>
